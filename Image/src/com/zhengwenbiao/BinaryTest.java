...
public class BinaryTest {
	...
	public static void main(String[] args) throws IOException {
		...
		BufferedImage bi = ImageIO.read(new File(
				"D:\\workspace\\Image\\bin\\新建文件夹\\" + 2 + ".jpg"));// 通过imageio将图像载入
		BufferedImage nbi = changeGrey(bi);
		BufferedImage lbi = median(bi);
		BufferedImage b1 = lineGrey(lbi);
		// BufferedImage b2 = grayFilter(b1);
		BufferedImage b2 = sharp(lbi);
		ImageIO.write(lbi, "jpg", new File("D:\\workspace\\Image\\bin\\新建文件夹\\"
				+ 2 + ".jpg"));
	}

	public static BufferedImage lineGrey(BufferedImage image) {
	...
	/** 平滑缩放 */
	public BufferedImage scaling(double s, BufferedImage image) {
	...
	public BufferedImage scale(Float s, BufferedImage image) {
	...
	public static BufferedImage sharp(BufferedImage nbi) throws IOException {
		int iw = nbi.getWidth();
		int ih = nbi.getHeight();
		int[] pixels = new int[iw * ih];

		PixelGrabber pg = new PixelGrabber(nbi.getSource(), 0, 0, iw, ih,
				pixels, 0, iw);
		try {
			pg.grabPixels();
		} catch (InterruptedException e) {
	...
	public static BufferedImage changeGrey(BufferedImage bi) {
		int iw = bi.getWidth();
		int ih = bi.getHeight();
		int[] pixels = new int[iw * ih];
		PixelGrabber pg = new PixelGrabber(bi.getSource(), 0, 0, iw, ih,
				pixels, 0, iw);
		try {
			pg.grabPixels();
		} catch (InterruptedException e) {
